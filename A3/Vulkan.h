...
namespace A3
{
    ...
    public:
        ...
        // ======================================
        static TextureParameter sheenBRDFLUTParameter;
        static float calculateSheenLUTValue(float NoV, float roughness)
        {
            return 1.0f - roughness * 0.5f * (1.0f - NoV);
        }

        static void createSheenBRDFLUTTexture(VulkanRenderBackend& vulkanBackend)
        {
            const uint32_t LUT_DIM = 256;
            std::vector<float> pixels(LUT_DIM * LUT_DIM * 4);
            //
            for (uint32_t y = 0; y < LUT_DIM; ++y)
            {
                float roughness = (float)y / (LUT_DIM - 1);
                for (uint32_t x = 0; x < LUT_DIM; ++x)
                {
                    float NoV = (float)x / (LUT_DIM - 1);

                    float sheenE = calculateSheenLUTValue(NoV, roughness);

                    uint32_t index = (y * LUT_DIM + x) * 4;
                    pixels[index + 0] = 0.0f;
                    pixels[index + 1] = 0.0f;
                    pixels[index + 2] = sheenE;
                    pixels[index + 3] = 1.0f;
                }
            }

            // https://github.com/DassaultSystemes-Technology/EnterprisePBRShadingModel/blob/master/res/Sheen_E.exr
            sheenBRDFLUTParameter = createTexture(
                vulkanBackend,
                "../Assets/glTF_sample_models/Sheen_E.exr",
                static_cast<uint32_t>(VK_FORMAT_R32G32B32A32_SFLOAT),
                LUT_DIM, LUT_DIM, pixels.data());

            std::cout << "=======================" << std::endl;
            std::cout << "sheenBRDFLUIParameter._index : " << sheenBRDFLUTParameter._index << std::endl;
        }
        // ======================================
    };
    ...
    struct MaterialParameter
    {
        ...
        // ======== using sheen material ========
        // ======================================
        Vec3 _sheenColorFactor = Vec3(0);
        float _sheenRoughnessFactor = 0;
