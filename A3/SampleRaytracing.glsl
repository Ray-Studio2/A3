...
layout( binding = 5 ) uniform SampleQuality {
	uint maxDepth;
	uint numSamples;
} sq;
...
void main()
{
    ...
    {
        vec3 sampleDir = uniformSampleSphere(gl_LaunchIDEXT.xy, i, payload.depth);
    ...
    payload.hitValue = accumulatedRadiance / float(numSampleByDepth);
}
#endif
