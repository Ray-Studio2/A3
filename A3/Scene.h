...
struct Animation
{
	...
	void sample(const float deltaTime, const std::string& boneName, Vec3& outTranslation, Vec4& outRotation, Vec3& outScale)
	{
		...
		// TODO: Binary Search
		const uint32 translationCount = static_cast<uint32>(animData._translation._time.size());
		for (uint32 i = 0; i < translationCount; ++i)
		{
			if (_currentTime > animData._translation._time[i])
				continue;
		...
		const uint32 rotationCount = static_cast<uint32>(animData._rotation._time.size());
		for (uint32 i = 0; i < rotationCount; ++i)
		{
			if (_currentTime > animData._rotation._time[i])
				continue;
		...
		const uint32 scaleCount = static_cast<uint32>(animData._scale._time.size());
		for (uint32 i = 0; i < scaleCount; ++i)
		{
			if (_currentTime > animData._scale._time[i])
				continue;
